...
const updateCart = async (product_id, user_id, quantity, setQuantity) => {
  try {
    ...
    const currenProducts = await Promise.all(currentCart.map((cart) => cart.products.id))
    const currenProductsIdx = currenProducts.indexOf(Number(product_id))
    
    if (currenProductsIdx === -1) {
      await cartDao.addNewItemToCart(Number(product_id), Number(user_id), Number(quantity))
    } else if (currenProductsIdx !== -1 && !setQuantity) {
      await cartDao.addMoreItemToCart(Number(product_id), Number(user_id), Number(quantity), Number(currentCart[currenProductsIdx].quantity))
    } else if (currenProductsIdx !== -1 && setQuantity) {
      await cartDao.updateItemInCart(Number(product_id), Number(user_id), Number(setQuantity))
    }
